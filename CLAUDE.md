...
- **⚠️ FEATURE-PIPELINE BUG FOUND + FIXED (2026-06-19, commit `e40b9d2`) — the most important find of the night.** The cheap edge probe + a leakage audit surfaced it: `data_loader.get_technical_features` ended with `df.dropna()`, and pandas-ta's Ichimoku **chikou span `ICS_26` = `close.shift(-26)`** carries trailing NaNs ⇒ dropna silently dropped the **most recent ~26 rows** ⇒ the scanner was fed **~26-day-STALE features** (raw 2026-06-18 vs features 2026-05-12). ICS is also a **future-close LOOKAHEAD** (inflated the GIA edge-probe AUC ~0.09; 0.70→0.61 without it). **Blast radius:** every decision via `get_technical_features` — **Forex (trend gate AND scan) + GIA (tactical scan)** — in BOTH backtest and live (live = stale-but-real past data, not future-leak). **ISA/SIPP allocation books UNAFFECTED** (they decide via the macro gate + real-time `get_latest_price`, not `get_technical_features`). **Fix:** drop the chikou (`ICS*`) BEFORE dropna ⇒ features current (gap 0); other Ichimoku lines kept; 125 tests pass. **IMPLICATIONS (honest):** (1) **Forex & GIA backtests ran on 26-day-lagged signals → MUST be RE-VALIDATED** — the prior **Forex "PASS" (PF 2.35/MAR 0.54) + the GFC Forex PASS are SUSPECT** until re-confirmed (re-validation running on corrected features). ISA/SIPP + all regime-gate results stand (gate path unaffected). (2) **The corpus is contaminated** (stale + ICS-leaked markers) → **regenerate**. (3) The "GIA promising AUC 0.70" probe finding is **SUPERSEDED** (mostly ICS leakage on stale features; re-probe after regen).
- **🔴 RE-VALIDATION RESULT ON CORRECTED FEATURES (2026-06-20) — GIA + FOREX FAIL; the "ALL FOUR BOOKS VALIDATED" milestone is RETRACTED.** Re-ran both windows on the fixed (current, no-lookahead) feature path. Confirmed rigorously that `get_technical_features` is used ONLY by `adapters/asset_fx.py` (Forex) + `layers/ml_scanner.py` (GIA tactical scan) — NOT by the allocation path / gate / engine, so **ISA + SIPP are genuinely UNAFFECTED and remain validated** (their gate+real-time-price path never touched the bug). The two tactical, feature-driven books, however, were riding the bug: **FOREX → FAIL** (corrected: GFC PF 1.18 / MAR 0.07 / MC 32%; in-sample PF 1.25 / MAR 0.10 — vs the stale-feature "PASS" of PF 2.35 / MAR 0.54); **GIA → FAIL** (corrected: Sortino 0.32-0.38, win ~46%, PF ~1.7 — vs stale Sortino 1.34 / win 69% / PF 7.9). The ICS future-close lookahead had INFLATED their backtests; remove it + the staleness and the tactical edge largely evaporates. (ISA/SIPP showed garbage numbers in THIS particular re-run — realized_pnl 0, MC 1e5% — but that's a separate HARNESS artifact: the canon-mode allocation books used their young per-book universes (SMGB.L etc.) in the pre-2020 GFC window where those tickers don't exist; NOT a real ISA/SIPP result. Their real proxy-based validations stand.) **NET STATUS: 2 of 4 books validated (ISA + SIPP allocation). GIA + Forex now need genuine strategy rework on corrected features** — which deeply reinforces the project's core lesson: the real edge is the REGIME GATE + diversified allocation, NOT feature-based tactical scanning (the scanner "had no edge"; the bug had been masking that for GIA + Forex). **Operator decisions (morning):** (a) GIA likely should become an ALLOCATION book like ISA (its tactical edge was bug-inflated); (b) Forex needs a genuinely re-thought signal (the trend-on-markers approach fails on honest features); (c) the live paper loop keeps accruing GIA/Forex on corrected features (harmless — they can't pass the live gate so can't go live; the data is now honest for redesign); (d) regenerate the corpus on corrected features. Better found now, on paper, than after going live.
- **✅ GIA REWORKED → ALLOCATION + validated (2026-06-20, commit `ff7289a`).** GIA switched from the (bug-inflated, de-validated) tactical scanner to a regime-gated aggressive US-growth ALLOCATION — held US AI-megacap basket (NVDA/MU/AVGO/TSM/ASML/ANET/VRT/GEV) + gold crash-diversifier (SGLN.L) + global ballast (VWRP.L); dropped unsafe-to-hold names (IREN/CRML microcaps, 3LNV.L 3x). Low turnover also suits GIA's CGT mandate. Validated on corrected features (proxies QQQ/SOXX/SPY/GLD): **in-sample PASS** (win 60%, PF 1.95, MAR 0.54, walk-forward 4.41); GFC survives (67% positive months, MAR 0.17 — same accepted "below-bar in the GFC, no lost decade" tail as ISA/SIPP). **Validation leak fixed alongside:** the allocation CURVE profile was inheriting trade-level `min_sortino=1.0` from the ISA/GIA base profiles via `replace()` — a trade metric that doesn't fit a buy-hold curve (SIPP never had it; ISA happened to pass). MAR/Calmar is the intended curve risk-adjusted gate; cleared `min_sortino` for allocation books. This flipped GIA in-sample FAIL→PASS (it failed ONLY the leaked bar; volatile-but-compounding aggressive book: MAR 0.54 with monthly-Sortino 0.29 — MAR is the honest measure). **STATUS NOW: 3 of 4 books validated (ISA/SIPP/GIA, all regime-gated allocation). Forex remains the one book needing a re-thought signal** (its tactical EMA/ADX trend fails on corrected features). NOTE: with all 3 equity books allocation, NO book uses the ML scanner (`ml_scanner.py`) live — only Forex uses `get_technical_features` now.
- **🔵 FOREX RE-THINK (2026-06-20) — no reliable DAILY edge; the answer is INTRADAY.** Dove into a re-thought FX signal per the operator. Tested TWO principled daily approaches on corrected features, both FAIL: (1) the existing **EMA/ADX trend** (corrected: PF 1.2 / MAR 0.10); (2) **TSMOM** 12-month time-series momentum (GFC PF 0.97 / MAR −0.01 negative; in-sample PF 1.15 / MAR 0.13). Stopped there deliberately — testing more daily variants risks data-mining a spurious pass. **Honest conclusion: the USD majors have no reliable DAILY edge for us** (consistent with FX-major efficiency + the project lesson that the edge is macro-regime, not per-instrument daily signals). **The Forex re-think therefore CONVERGES with the intraday workstream:** FX's real opportunity is **intraday (session / time-of-day)** — the operator's own idea (Phase 1 observe-only already shipped; Phase 2/3 is the merged intraday-backtest+execution sprint). **RECOMMENDATION:** PARK the daily FX book (no validated daily edge — don't trade it daily / keep the live track record to validated books) and make the **intraday FX build the next major sprint**; Forex stays the 1 unvalidated book until that's done. STATUS: 3 of 4 validated (ISA/SIPP/GIA allocation); Forex pending the intraday sprint. (Experiment harness: `_exp_sizing.py` `fx_tsmom` mode; production `FxAdapter` unchanged pending the operator's call on park vs intraday-now.)
- **✅ FX PARKED + HOUSEKEEPING DONE (2026-06-20, commit `4391b49`, operator-approved).** (1) **FX book PARKED** — `gate_min: 999` (unreachable) so it never opens new entries, until the intraday strategy replaces it (production `FxAdapter` left intact for that). (2) **Forward ledger archived** (`nova_ledger.archived-20260620-preGIAalloc-FXparked.db`) → fresh `nova_ledger.db` for a clean track record under the validated 3-book config (ISA/SIPP/GIA allocation; FX parked). The daily cron resumes accrual on this clean state. (3) **Corpus REGENERATED** on corrected (no-lookahead) features + the all-allocation config (ISA/SIPP/GIA proxies, FX excluded) — replacing the contaminated old corpus. NOTE: the corpus is now an **allocation / regime-timed decision dataset** (the bug-inflated tactical scanner is retired), so it's **cleaner but sparser** than the old 26.5k (which was 94% GIA scanner records) — which also means the "rich tactical dataset to fine-tune on" premise is weaker; the model-training value now leans toward RAG / the LLM auditor over a large supervised set. **STATUS: clean validated 3-book system (ISA/SIPP/GIA), FX parked pending the INTRADAY FX SPRINT (the next major workstream — merged Phase 2/3 per `research/INTRADAY_FX_TIMING_SCOPE.md`).**
- **🟢 CLEAN LIVE-PAPER ACCRUAL BEGUN (2026-06-20).** First cycle on the validated 3-book config (ISA/SIPP/GIA allocation) + FX parked, logged to the fresh `nova_ledger.db` via live TWS (read-only). Confirmed FX trades nothing (parked) and the books gate-gate correctly (held cash — regime gate <75 today). The daily cron continues; this is the "protect the core, let calendar time accrue" phase — the only remaining validation the regime gate needs is **live forward data (3-6 months)**. Per the protect-the-core strategy (operator + Gemini aligned): the 3 books' logic is **frozen** — monitor (dashboard / `python run_paper.py --cycles 0`), don't fiddle.
- **🔍 ML-SCANNER REVISIT — honest verdict: keep RETIRED (2026-06-20, operator-invited).** Re-examined whether the 32-indicator ML scanner holds any value to current status. PREDICTIVE value: NONE — the leakage-corrected edge probe showed markers add no robust OOS edge beyond the regime gate (the apparent GIA AUC 0.70 was ICS lookahead), and there is NO tactical book left to deploy it in (all 3 equity books are allocation). Resurrecting it would re-introduce overfitting/noise risk. DESCRIPTIVE value: the 32 indicators are RETAINED as CONTEXT — they characterize market state for the LLM auditor's Inference Context Bundle and the future RAG explainer (e.g. "RSI/ADX/regime at decision time"), which is legitimate. Net: **scanner stays retired; indicators stay as auditor/RAG context, not as a trading signal.** Not resurrected.
- **🔵 FX INTRADAY — Phase 2 PROBE done (2026-06-20, `backtest/fx_intraday_probe.py`, commit `bfbf90b`).** Operator believes FX is profitable intraday; tested it honestly BEFORE building any intraday engine, on ~2.75y hourly majors (yfinance cap), costs charged (2-pip round-trip), high-bar/abandon-readily discipline. THREE angles converge on **NO cost-surviving edge**: (A) session/day-of-week structure = sub-cost noise (weak Mon+/Fri− tilt, magnitudes ≪ cost); (B) **Asian-range breakout** (the canonical intraday FX strategy) PF 0.95 / Sharpe −0.27 net; (C) **session momentum** raw cross-session mean ≈ 0 (no exploitable autocorrelation), net −2.3bp. The obvious/canonical intraday FX edges are NOT present in this window. Lone recurring positive = **USDJPY** (small, all 3 tests) — but it's 1 of 6 and sits in the 2023-26 yen-carry-unwind regime; cherry-picking it = data-mining. CAVEATS: only ~2.75y (no crash stress test), 3 archetypes tested (not exhaustive — but the general STRUCTURE test shows the raw material isn't there, and mining more archetypes on 2.75y is the trap). **VERDICT: FX stays PARKED — lean ABANDON.** Not built further (no Phase 3) absent a glaring edge. Remaining untested exotica (news-time, carry-overlay, order-flow needing tick/L2 we don't have, intraday ML) are high-data-mining-risk / out-of-reach — operator's call to pursue a specific hypothesis, but not worth autonomous time. Equity books untouched. **This effectively closes the FX workstream unless the operator wants a specific hypothesis chased: the project's alpha is the regime-gated equity allocation; FX has not earned a place.**
- **🔵 FX MACRO-REFRAME tested (2026-06-20, Gemini's 3 paths scrutinised).** Gemini proposed rebuilding FX as a MACRO-regime book (not technical): (1) carry/policy via rate differentials; (2) risk-proxy allocation (AUD/JPY, NZD/JPY) gated by the macro gate; (3) event-driven (economic-calendar releases). Data feasibility: AUD/JPY+NZD/JPY have 22y history (Path 2 GFC-testable ✅); foreign 2y yields NOT on yfinance (Path 1 needs a FRED integration + carry-accrual modeling 🟠); no economic-calendar/consensus/intraday data (Path 3 infeasible 🔴). **TESTED Path 2** (regime-gated long AUD/JPY+NZD/JPY, both windows): **FAILS** — GFC PF 0.39 / win 6.9% / MAR −0.09 (catastrophic), in-sample PF 1.34 / MAR 0.17 (<0.30). AUD/JPY is a concentrated higher-beta risk-proxy that craters in risk-off and the daily gate LAGS the crash — strictly WORSE than the diversified (gold-buffered) equity allocation. **PUSHBACK on the "USDJPY clue":** the 2023-26 USDJPY-up move = the carry trade WORKING in a calm/risk-on regime, NOT a persistent long-risk-proxy edge; and carry IS the risk-on trade that just demonstrated catastrophic crash behavior (AUD/JPY 2008). So a macro FX book gated by the same LAGGING equity-based gate is CORRELATED with the equity risk-on exposure AND crashes harder — it duplicates+amplifies the bet with worse tails, it doesn't diversify. **VERDICT: tested daily-technical (fail), intraday-technical (fail), AND the macro risk-proxy reframe (fail, worse than equities) → RECOMMEND ABANDON FX.** Path 1 (carry) is the only untested real-edge option but needs new infra (FRED rates + carry accrual) AND has the same crash-lag vulnerability (carry = "pennies in front of a steamroller", gate lags the steamroller) — low expected value. The equity allocation already IS the safer, diversified risk-on exposure. FX abandoned unless the operator explicitly wants Path 1 (carry) scoped with eyes open on the build + the crash-lag pitfall.
- **🔵 FX CROSS-SECTIONAL / DOLLAR-NEUTRAL probe (2026-06-20, `backtest/fx_xsectional_probe.py`) — the structurally-different route, also no edge.** Everything that failed was DIRECTIONAL + risk-on-correlated; the escape is a LONG-SHORT, dollar-neutral cross-sectional book (rank currencies vs each other), which is market-neutral ⇒ no crash-lag, no equity correlation. Tested the cheap leg (cross-sectional 12m momentum, long top-2 / short bottom-2 of the 6 majors, monthly, costs charged): **the construction WORKS (SPY corr −0.05, genuinely diversifying) but has NO RETURN** — Sharpe −0.05, total −12%/19y, flat in EVERY sub-period (2007-12 incl GFC, 2013-19, 2020-26). It diversifies nothing. **FX NOW COMPREHENSIVELY EXPLORED across all major edge archetypes: daily-technical (fail), intraday-technical (fail), macro-directional risk-proxy (fail), cross-sectional/factor momentum (fail).** The only legs untested are carry + value — which need significant new data infra (FRED foreign rates + CPI/REER) for premia that have DECAYED post-2008, and carry retains crash risk even cross-sectionally. **DEFINITIVE VERDICT: ABANDON FX.** Every reasonable archetype fails; the equity allocation is the alpha; building carry/value factor infra is low-EV. Only revisit if the operator has strong conviction for the full institutional carry+value cross-sectional book (scoped on request, low EV). FX stays parked → effectively retired.
- **✅ FX WORKSTREAM CLOSED (2026-06-20, commit `ecdeb34`, operator + Gemini consensus).** FX removed from the LIVE loop (`run_paper.py` excludes `ibkr_forex_margin`; only `AllocationAdapter` wired — the tactical EquityAdapter + FxAdapter are retired from live since all 3 equity books are allocation). The live system is now **purely the 3 validated equity allocation books (ISA/SIPP/GIA)**. FX book definition + research probes retained for the record/reversibility. **FORWARD FOCUS (settled):** (1) **let the 3 equity books' LIVE PAPER track record MATURE — 3-6 months, the priority — monitor (dashboard / `python run_paper.py --cycles 0`), don't fiddle;** (2) **SaaS side = LLM Auditor + RAG** (RAG-first, when the operator directs). NO more predictive-ML or FX engineering — the alpha is the regime-gated diversified equity allocation, full stop. This is the project's converged, honest end-state for the research phase.
