...
3. **Fine-tune / model-training pipeline (NOT built; RAG-first by design).** Corpus + track record accrue, but an actual local-LLM fine-tune isn't built. Scope + build when the live corpus is substantial AND the operator is ready for the GCP/Vertex side. RAG-first (`saas/` bridge), fine-tune later as the moat.
