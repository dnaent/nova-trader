...
## Phase A — PRELIMINARY RESULT (2026-06-19, `backtest/edge_probe.py` on `nova_corpus.db`)
Ran the cheap probe (acted-only, walk-forward TimeSeriesSplit ×5, gate-only vs gate+markers, HistGBM):
- **GIA — PROMISING but UNCONFIRMED.** gate+markers mean OOS AUC **0.70** (folds 0.80/0.61/0.51/0.80/0.77) vs gate-only 0.573, with a **pooled +0.38R expectancy uplift** (model-top-half +1.25R vs all +0.87R). Real predictive power in 4 of 5 folds, but one fold collapses to 0.51 → **unstable, not robust on ~1,068 acted samples.** Notably this *differs* from the old "scanner had no edge" finding because the target differs — here we predict the **actual trade R>0 outcome** (which bakes in the stop/take/exit structure), not the old 5-day-breakout direction. So the markers may predict "will this trade hit take before stop" better than "will price move."
- **Forex — NO reliable edge.** mean AUC 0.56 (folds 0.43-0.71), barely above gate-only (0.53). FX's edge is the trend strategy, not the markers. Drop markers from any FX selection model.
- **Caveats (do NOT over-trust):** acted-only + selection-biased; small sample; single corpus/regime per fold; **a leakage audit is required before believing AUC 0.7** — especially pandas-ta **Ichimoku's forward-projected spans** (senkou A/B are plotted ahead; confirm the snapshot uses only point-in-time values). The high-but-unstable AUC could be a real regime-dependent selection edge OR small-fold overfit.
- **Refined recommendation:** the result **justifies Phase 0 for GIA specifically** (counterfactual-label the ~24k not-acted GIA candidates → a large, unbiased dataset → re-probe; this is exactly what would stabilise a 0.51-0.80 fold spread) **plus a leakage audit of the markers**. **Do NOT bother with Phase 0/B for Forex** (no marker edge). The regime gate remains the primary, reliable edge; GIA markers are a *candidate* trade-selection refinement worth confirming, not a proven one.
